...
1.  
Una lista enlazada es una estructura de datos en la que cada elemento, llamado nodo, guarda la información que quiero almacenar y también una referencia (o enlace) al siguiente nodo de la lista. Esto hace que los elementos no tengan que estar uno al lado del otro en la memoria, sino que pueden estar en distintas posiciones, siempre y cuando estén conectados entre sí por esos enlaces.
...
7.  
En este código no hay fugas de memoria porque cada vez que la lista se destruye o se limpia se llama a la función clear(), que recorre todos los nodos, los elimina con delete y libera la memoria que ocupaban.
...
1.   
En una implementación manual de un stack en C++ la memoria se gestiona de manera dinámica, ya que cada vez que hago un push se usa new para crear un nodo en el heap y cada vez que hago un pop se usa delete para liberar ese espacio. Esto me permite tener control sobre cómo se guardan y eliminan los elementos de la pila, pero también implica riesgos: si no libero bien la memoria puedo generar fugas y, si llego a acceder a un nodo ya eliminado, podría tener errores graves en la ejecución. Además, este proceso no es tan rápido porque el uso constante de new y delete hace que el programa tenga un costo extra en rendimiento comparado con estructuras que manejan memoria de forma más directa.

2.  
Es importante liberar la memoria cuando se desapila un nodo en un stack porque, al usar memoria dinámica con new, cada nodo ocupa un espacio en el heap que no se recupera automáticamente. Si no uso delete al desapilar, esos espacios se van acumulando como fugas de memoria, lo que significa que el programa mantiene reservada memoria que ya no utiliza. En aplicaciones pequeñas puede no notarse de inmediato, pero en programas que se ejecutan por mucho tiempo o que trabajan con muchos datos, estas fugas terminan llenando la memoria disponible y pueden ralentizar la ejecución o incluso hacer que el programa se bloquee. Liberar la memoria asegura que el sistema pueda reutilizar esos recursos y que el stack funcione de manera segura y rapida.

3.  
La diferencia principal entre usar un std::stack de la STL y crear un stack manualmente está en el nivel de abstracción y control. Con std::stack no tengo que preocuparme por cómo se almacenan los elementos ni por la gestión de la memoria, ya que la librería se encarga de todo esto de forma segura y eficiente, evitando fugas y errores comunes con punteros. Esto simplifica mucho la implementación y me permite concentrarme en la lógica del programa en lugar de en los detalles de bajo nivel. En cambio, cuando creo un stack manualmente debo gestionar la memoria con new y delete, controlar cómo se enlazan los nodos y asegurar que todo se libere al final. Esto da más control y al ser mas basico es mas facil de aprender cómo funciona internamente una pila, pero también aumenta el riesgo de cometer errores y hace que el desarrollo sea más complejo.

4.  
La estructura de un stack afecta directamente el orden en que se accede y se eliminan los elementos, ya que funciona con la lógica LIFO (Last In, First Out): el último elemento en entrar es siempre el primero en salir. Esto significa que no puedo acceder libremente a cualquier elemento de la pila, sino que solo trabajo con el que está en la cima. Esa característica lo hace muy eficiente para problemas donde necesito revertir un orden o llevar un seguimiento de pasos recientes. Por ejemplo, es ideal para implementar la función de deshacer en editores. En general, la naturaleza LIFO simplifica la gestión cuando lo importante es recordar el último elemento agregado y procesarlo antes que los demás.
...
class Node {
public:
    Circle data;    // Se guarda el objeto completo aquí
    Node* next;
...
```
6.  
En una implementación manual de una queue en C++, la memoria se maneja creando un nuevo nodo cada vez que encolamos (con new) y liberándolo al desencolar (con delete). Esto implica que yo misma debo estar pendiente de liberar los nodos para no dejar fugas de memoria, ya que si olvido hacerlo la cola se va llenando de basura que nunca se libera, y en programas que duran mucho tiempo esto puede ser grave. También debo tener cuidado con punteros colgantes, porque si elimino un nodo pero sigo apuntando a él, el programa puede fallar. Usar punteros crudos funciona, pero es más propenso a errores. Una forma más segura sería usar punteros inteligentes como unique_ptr, que liberan la memoria automáticamente y hacen que mi implementación sea más confiable. En términos de eficiencia, las operaciones de encolar y desencolar son rápidas porque solo enlazan o desenlazan nodos, pero el hecho de estar usando new y delete constantemente puede hacer más lento el programa en comparación con usar estructuras de la STL como std::queue, que ya están optimizadas.

7.  
Implementar una queue tiene más retos de memoria que un stack, porque en el stack solo me preocupo por un puntero al tope, mientras que en la cola debo controlar dos: el front y el rear. Eso significa que al encolar debo enlazar correctamente el nuevo nodo al final y actualizar rear, y al desencolar debo mover front al siguiente y liberar el nodo anterior. Si no actualizo bien ambos punteros, puedo dejar la estructura en un estado inconsistente, por ejemplo con un rear apuntando a memoria liberada o con un front que ya no existe. Además, cuando la queue queda vacía debo asegurarme de que tanto front como rear queden en nullptr, porque si no, al intentar acceder puedo tener errores graves. En resumen, el desafío está en coordinar dos punteros en lugar de uno solo, lo que aumenta las posibilidades de fugas de memoria o punteros colgantes si no soy cuidadosa.

8.  
La estructura FIFO de una queue influye directamente en los problemas donde el orden es fundamental, porque garantiza que el primer elemento en entrar sea también el primero en salir. Esto es clave en situaciones como colas de espera, procesos de impresión o sistemas de atención al cliente, donde sería injusto o caótico que alguien que llegó después fuera atendido primero. También en programación se usa para manejar tareas en orden de llegada, por ejemplo en la gestión de procesos de un sistema operativo o en el recorrido por niveles de un grafo (BFS). En estos casos, la queue asegura un flujo organizado y predecible, ya que los elementos se procesan exactamente en el orden en que aparecen, lo que sería difícil de lograr con una estructura LIFO como el stack.

9.  
Una forma de implementar una queue circular es usando un arreglo fijo y dos índices (front y rear) que se mueven de manera circular con el operador módulo. En lugar de dejar espacios vacíos cuando desencolo, la cola circular reutiliza esas posiciones, lo que la hace más eficiente en memoria que una cola lineal tradicional, donde el rear solo avanza y al final ya no se puede usar el espacio libre al inicio. La ventaja es que no necesito estar moviendo elementos ni creando nodos nuevos, porque todo se maneja dentro del mismo arreglo, aprovechando al máximo la capacidad. Para implementarla tendría que cambiar la lógica de encolar y desencolar, controlando bien cuándo la cola está llena o vacía y actualizando los índices con cuidado, pero con eso se consigue una gestión más eficiente y ordenada de la memoria.

10.  
Si no manejo bien los punteros front y rear en una queue, pueden aparecer varios problemas graves. Por ejemplo, si desencolo un nodo y no actualizo el front, pierdo la referencia al siguiente nodo y no hay forma de acceder a él, lo que genera una fuga de memoria. También puede pasar que el rear siga apuntando a un nodo que ya fue eliminado, y si trato de encolar algo nuevo a partir de ese puntero, el programa puede fallar. Otro error común es no poner ambos punteros en nullptr cuando la cola queda vacía, lo que hace que parezca que aún hay elementos y puede dar resultados inesperados. Para evitar esto, hay que ser cuidadosa actualizando los punteros en cada operación, asegurarse de que siempre apunten a nodos válidos y, cuando ya no haya elementos, resetearlos a nullptr. De esa forma, la estructura se mantiene consistente y se previenen fugas o accesos inválidos.